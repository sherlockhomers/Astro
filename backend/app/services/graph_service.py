...
class GraphService:
    ...
    def run_cypher(self, query: str, params: dict[str, Any]) -> tuple[bool, list[dict[str, Any]], str]:
        ...
        try:
            ...
            # default_access_mode='READ' + execute_read，由 driver 在事务层兜底拒绝写
    ...
    # 允许导出 Cypher 的根目录：backend/data/exports（不存在会创建）。
    # 任何尝试越出该目录的 output_path 一律拒，避免任意文件写。
    _CYPHER_EXPORT_ROOT = Path(__file__).resolve().parents[2] / "data" / "exports"

    def _safe_export_path(self, output_path: str) -> Path:
        candidate = Path(output_path)
        if not candidate.is_absolute():
            candidate = (self._CYPHER_EXPORT_ROOT / candidate).resolve()
        else:
            candidate = candidate.resolve()
        root = self._CYPHER_EXPORT_ROOT.resolve()
        try:
            candidate.relative_to(root)
        except ValueError as exc:
            raise ValueError(
                f"output_path 必须位于 {root} 下"
            ) from exc
        if candidate.suffix.lower() not in {".cypher", ".cql", ".txt"}:
            raise ValueError("output_path 必须以 .cypher/.cql/.txt 结尾")
        return candidate
    ...
    def export_cypher(self, output_path: str) -> tuple[bool, int, int, str]:
        ...
        try:
            safe_output = self._safe_export_path(output_path)
        except ValueError as exc:
        ...
        return True, len(entities), len(relations), f"Cypher 已导出到 {safe_output}"
